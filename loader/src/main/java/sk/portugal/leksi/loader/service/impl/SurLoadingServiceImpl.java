...
public class SurLoadingServiceImpl implements LoadingService {
    ...
    private static final String FILTER = ""; // " AND portugal in ('acompanhante')";
        ...
        for (Word tr: wordList) {
            for (int i =  tr.getWordTypes().get(wType).getMeanings().size() - 1; i >= 0; i--) {
                ...
                if (se.getSynonyms().startsWith("##")) {
                    ...
                    prev.setSynonyms(StringUtils.stripEnd(prev.getSynonyms(), null) + " " + StringUtils.stripStart(se.getSynonyms().substring(2), null));
